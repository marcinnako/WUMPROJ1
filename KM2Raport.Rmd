...
Czesciej bogate osoby biora kredyty na nowy samochod, wakacje czy przebranzowanie.
Biedne odoby natomiast czesciej biora kredyty na potrzeby domowe jak nowy telewizor lub sprzet domowy


```{r}
num_data$purpose <- if_else( data$purpose %in% c("new_car", "education", "retraining", "business", "other" ), 1, 0)
```
...
Jezeli tak ma 1 jezeli bierze kredyt wspolnie ma 0 a przy ich braku -1

```{r}
num_data$other_debtors <- if_else(data$other_debtors %in% c("guarantor"), 1, if_else(data$other_debtors %in% c("co-applicant"), 0,-1))
```
...
```{r}
num_data$other_installment_plans <- if_else(data$other_installment_plans %in% c("bank"), 1, if_else(data$other_installment_plans %in% c("stores"), 0,-1))
```
...
```{r}
num_data$housing <- if_else(data$housing %in% c("rent"), 1, if_else(data$housing %in% c("own"), 0,-1))
```
...
Można z tego wyciągnąć informacji na dwie kolumny ## COOO? (Marcin)
