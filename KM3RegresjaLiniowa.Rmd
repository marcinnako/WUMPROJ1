...
Dane te były wczśniej przez nasz zespół odpowiednio przygotowane dzięki czemu możemy od razu przejść do trenowania naszego modelu.

## Metryka dokładności

Do pomocy w określeniu jakośći naszego modelu napiszemy funkcje która wyliczy % poprawnych klasyfikacji.
...
Sama fukcja nie przyjmuje odobno danych i celu treningowego tylko wszystkie dane w jednym pliku.
...
Wykorzystując naszą wcześniej zdefiniowaną metryke, znajdziemy jaki jest najlepszy punkt odcieńcia tak aby nasz model działał jak najlepiej.
Naturalnym punktem wydaje się być podział po 50% jednakże z powodu danych wejściowych możliwa jest przechylność modelu w jedną ze stron.
Sprawdzmy to teraz.
...
Nasz model uzyskuje najlepszą celność przy podziale na `r toString((1-best_split)*100)`% dla 1 (Dobry kient) oraz  `r toString((1-best_split)*100)`% dla 0 (Zły kient)
